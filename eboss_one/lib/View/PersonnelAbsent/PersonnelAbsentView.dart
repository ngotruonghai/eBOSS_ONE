...
import '../../Model/PersonnelAbsent/ListPersonnelAbsentModel.dart'
...
import 'ListYear_PopView.dart';
import 'LoadListMonth_PopupView.dart';
import 'PersonnelAbsentCreateView.dart';
import '../../Model/PersonnelAbsent/MonthModel.dart' as MonthModel;
import '../../Model/PersonnelAbsent/YearModel.dart' as YearModel;
import 'PersonnelAbsent_ChiTietNghiPhep_View.dart';

class PersonnelAbsentView extends StatefulWidget {
  PersonnelAbsentView({super.key});

  @override
  State<PersonnelAbsentView> createState() => _PersonnelAbsentView();
}

class _PersonnelAbsentView extends State<PersonnelAbsentView> {
  ...
  Future<int> API_LoadDanhSachDOnPhep() async {
    ...
    final responses = await NetWorkRequest.PostJWT(
        "/eBOSS/api/PersonnelAbsent/GetListListPersonnelAbsent", request);
    final Response =
        ModelDanhSachDonPhep.ListPersonnelAbsentModel.fromJson(responses);
        ...
        actions: [
          IconButton(onPressed: () async {
            final result = await Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => PersonnelAbsentCreateView()),
            );
            if (result == 'refresh') {
              _refreshData();
            }
                                  ...
                                  children: DanhSachDonPhep.map((item) {
                                    return Padding(
                                        padding: EdgeInsets.symmetric(vertical: 5, horizontal: 5),
                                            child: InkWell(
                                              onTap: () async {                                                
                                                print("absentAID: ${item.absentAID}");
                                                final result = await Navigator.push(
                                                  context,
                                                  MaterialPageRoute(
                                                    builder: (context) => PersonnelabsentChitietnghiphepView(                                                      
                                                        absentAID: item.absentAID.toString(),
                                                        absentID: item.absentID.toString(),
                                                        nameVietnamese: item.nameVietnamese.toString(),
                                                        statusID: item.statusID.toString(),
                                                        listOfAbsentType: item.listOfAbsentType.toString(),
                                                        ...
                                                        descriptionAbsent: item.descriptionAbsent.toString(),
